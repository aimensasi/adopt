<div>
	...
	<div class="w-2/3 mx-auto overflow-hidden bg-white shadow sm:rounded-lg">
			...
			<dl>
				...
				@if (auth()->user()->hasRole('Admin'))
					<div class="px-4 py-5 bg-gray-50 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
						...
						<dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
							{{ $adopter->email }} - {{ $adopter->phone_number }}
						</dd>
					</div>
				@endif
				<div class="px-4 py-5 bg-white sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
		...
		<div class="flex w-full">
			@if (auth()->user()->hasRole('Admin'))
				<button class="px-4 py-2 my-4 mt-10 ml-auto mr-5 text-white bg-red-600 rounded-full shadow-md btn" @if($status != null) disabled @endif wire:click.prevent="onDeleteAccount">
					Delete Account
				</button>
			@else
